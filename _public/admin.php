...
$Route->add("/admin/approve-account/{accid}", function ($accid) {
	...
	$mailbody = "<p>Congratulations <strong>{$fullname}</strong>!</p>
            <p>Your account has now been approved and ready for use.</p>
			<p>Use the details below to login to your account. You will be mandated to change your login password on first login.</p>
			<p>
			...
			<p>Thank you for choosing Innovative Management & Corporate Bank.</p>
            ";
...
$Route->add('/banco/admin/create', function () {
	...
	if ($newaccid) {
		...
		$ThisUser = $Banco->UserInfo($newaccid);
		...
		$subject = "Welcome to Innovative Management & Corporate Bank";
		$mailbody = "<p>Congratulations <strong>{$fullname}</strong>!</p>
		<p>Your account profile with Innovative Management & Corporate Bank has been created. Kindly log in to your account to change your default password.</p>
...
$Route->add('/banco/admin/profile', function () {
	...
	$tax_transfer_error = 0;
	if (isset($Data->tax_transfer_error)) {
		$tax_transfer_error = 1;
	}
	$Banco->SetUserInfo($accid, "tax_transfer_error", $Data->tax_transfer_error);
	$Banco->SetUserInfo($accid, "tax_error_code", $Data->tax_error_code);

	$aml_transfer_error = 0;
	if (isset($Data->aml_transfer_error)) {
		$aml_transfer_error = 1;
	}
	...
	if ($_FILES["avatar"]['size'] > 0) {
		...
		if ($handle->processed) {
			$Banco->SetUserInfo($accid, "avatar", $handle->file_dst_pathname);
			$handle->clean();
		}
	}
...
$Route->add('/ajax/{cmd}', function ($cmd) {
	...
	if ($cmd == 'profile') {
		...
		if ($trtype == "credit") {
			...
			<p>This is to inform you that a transaction has occurred on your account with Innovative Management & Corporate Bank with details below:</p>
			<p>
